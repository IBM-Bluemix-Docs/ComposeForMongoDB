...
資料庫的每日備份是自動排定的。若要檢視現有備份，請導覽至服務儀表板的*管理* 頁面。
  ![備份](./images/mongodb-backups-show.png "可用備份清單")
...
請使用下列步驟，以使用 {{site.data.keyword.cloud_notm}} CLI 將備份從執行中服務還原至新的服務。 
...
```
  _SERVICE_ 欄位應該是 compose-for-mongodb，而 _PLAN_ 欄位應該是 Standard（標準）或 Enterpris（企業），視您的環境而定。_SERVICE\_INSTANCE\_NAME_ 是您將放置新服務名稱的位置。_source\_service\_instance\_id_ 是備份來源的服務實例 ID；其取得方式是執行 `ibmcloud cf service DISPLAY_NAME --guid`，其中 _DISPLAY\_NAME_ 是備份來源的服務名稱。 
...
例如，將舊版的 {{site.data.keyword.composeForMongoDB}} 服務還原為執行 MongoDB 3.4.10 的新服務如下所示：
```
